        ...
        if (true) {
            if (this.facingPos != null) {
                if (var1.action == Action.RIGHT_CLICK_BLOCK) {
                    if (!this.facingPos.equals(bogdanqol.mc.objectMouseOver.getBlockPos())) {
    ...
    private void clickSecret(BlockPos var1) {
        if (var1 != null) {
            try {
                ...
                bogdanqol.mc.playerController.onPlayerRightClick(MinecraftUtils.getPlayer(), bogdanqol.mc.theWorld, MinecraftUtils.getPlayer().inventory.getCurrentItem(), this.facingPos, bogdanqol.mc.objectMouseOver.sideHit, bogdanqol.mc.objectMouseOver.hitVec);
