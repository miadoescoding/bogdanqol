        ...
        if (true) {
            if (Configs.BurrowHelper) {
                ...
                if (this.solution != null && this.solution.getY() < 5 && var2 - lastUpdate > 1000L) {
                    ...
                    this.solution = new BlockPos(var4, calcHighestGrass(var4, var5), var5);
                }
        ...
        if (true) {
            if (Configs.BurrowHelper) {
                if (var1.packet instanceof S2APacketParticles) {
                    ...
                    if (var2.getParticleType() == EnumParticleTypes.FIREWORKS_SPARK && MathUtils.equal(var2.getXOffset(), 0.0) && MathUtils.equal(var2.getYOffset(), 0.0) && MathUtils.equal(var2.getZOffset(), 0.0)) {
                        ++this.particleCount;
                        if (this.awaiting) {
            ...
            if (Configs.BurrowHelper) {
                if (var1.action == Action.RIGHT_CLICK_AIR || var1.action == Action.RIGHT_CLICK_BLOCK) {
                    ...
                    if (StringUtils.stripControlCodes(var2.getDisplayName()).contains("Ancestral Spade")) {
                        ...
                        double var3 = MathUtils.validPitch(MathUtils.getPitch());
