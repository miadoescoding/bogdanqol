...
import com.bogdan.qol.Config.Setting.*;
...
public class Config {
    ...
    private static final String fileName = "config/bogdanqol.cfg";

    public static void save() {
        try {
            HashMap var0 = new HashMap();
            Iterator var1 = bogdanqol.settings.iterator();
            ...
            Files.write(Paths.get("config/bogdanqol.cfg"), var4.getBytes(StandardCharsets.UTF_8));
    ...
    public static void load() {
        try {
            File var0 = new File("config/bogdanqol.cfg");
            if (var0.exists()) {
                BufferedReader var1 = Files.newBufferedReader(Paths.get("config/bogdanqol.cfg"));
                Set<Map.Entry<String, JsonElement>> set = new JsonParser().parse(var1).getAsJsonObject().entrySet();
                for (Map.Entry<String, JsonElement> entry : set) {
                    Setting setting = Config.getSetting(entry.getKey(), bogdanqol.settings);
                    if (setting != null) {
